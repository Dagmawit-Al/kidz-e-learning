...
const Course = ({ data }) => {
  return (
    <NavLink to={`/courses/book/${data.id}`} >
      ...
      <div class=" m-8 h-[100%] flex flex-col  max-w-sm border border-white rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 ">
        <a href="#">
          <img class="flex items-center w-[100%] rounded-t-lg"  src={data.src} alt="" />
        </a>
        <div class="p-5">
          <a href="#">
            <h5 class="bubblegum-sans-subheader mb-2 text-2xl text-center font-bold tracking-tight text-yellow-200 dark:text-white">{data.title}{" "}</h5>
          </a>
      ...
      {data.status ? (
        <span
        class="absolute rounded-full py-5 px-5 text-xl font-medium content-[''] leading-none grid place-items-center top-[2%] right-[2%] translate-x-1/4 -translate-y-4/4 bg-green-500 text-white min-w-[24px] min-h-[24px]">
        Available <br/> Now!
      </span>
      ) : (
        <span
    class="absolute rounded-full py-5 px-5 text-xl font-medium content-[''] leading-none grid place-items-center top-[2%] right-[2%] translate-x-1/4 -translate-y-4/4 bg-orange-500 text-white min-w-[24px] min-h-[24px]">
    Coming <br/> Soon!
  </span>
      )}
      {/* <span
